...
with open("fraud/fraudulent_abstracts.json") as f:
    data = json.load(f)

for i in tqdm(data):
    result = component_extract(data[i]["sentences"])
    print(result)
    with open("fraud/fraud_component_extract_record.txt", 'a', encoding="utf-8") as f:
        f.write(result)
